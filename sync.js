...
const CONFIG = {
    ...
    outputDir: path.join(__dirname, 'api-documentation')
};
...
const getWorkspaceId = async () => {
    ...
    try {
        ...
        const workspace = response.data.workspaces.find(w => w.name === CONFIG.workspaceName);
        if (workspace) {
            return workspace.id;
        }
        console.warn(`⚠️  Workspace "${CONFIG.workspaceName}" not found. Searching all collections directly.`);
        return null;
    } catch (error) {
...
const getCollectionId = async (workspaceId) => {
    ...
    if (!CONFIG.collectionName) {
        throw new Error('Please provide either COLLECTION_ID or COLLECTION_NAME in .env');
    }
    ...
    try {
        ...
        const collection = response.data.collections.find(c => c.name === CONFIG.collectionName);
        if (collection) {
...
const main = async () => {
    try {
        ...
        console.log(`✅ Collection JSON saved to api-documentation/collection.json`);
